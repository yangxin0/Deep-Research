...
from tools.creat_graph import MindMap as mindmap
...
class StormArticleGenerationModule(ArticleGenerationModule):
    ...
    def __init__(
        ...
        self.section_gen = AgtResSection(engine=self.article_gen_lm, tools = tools)
    ...
    def generate_section(
        ...
        outline_map = mindmap(section_outline)
...
class AgtResSection(dspy.Module):
    """Use the information collected from the information-seeking conversation to write a section through agentic reasoning."""

    def __init__(self, engine: dspy.LM, tools: list = []):
        super().__init__()
        # self.write_section = dspy.Predict(WriteSectionDeep)
        self.engine = engine
        self.tools = tools
        self.write_section = dspy.ReAct(WriteSectionDeep, tools=self.tools)
    def forward(
        self, topic: str, outline: mindmap, section: str, collected_info: List[Information]
    ):
        info = ""
        ...
        context = outline.graph_retrieval(f"How does the section: {section} contribute to the paper about: {topic}")

        with dspy.settings.context(lm=self.engine):
            section = ArticleTextProcessing.clean_up_section(
                self.write_section(topic=topic, info=info, context=context, section=section).output
            )
        return dspy.Prediction(section=section)


class WriteSectionDeep(dspy.Signature):
    """Write a Research Paper section based on the collected information. Consider how this section relates to the overall paper.
    ...
    context = dspy.InputField(prefix="How this section relates to the overall paper: ", format=str)
